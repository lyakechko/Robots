...
public class SecondScientist implements Runnable {
    ResourceFactory resourceFactory;
    List<SpareParts> listSecondScientist = new ArrayList<>();

    public List<SpareParts> getListSecondScientist() {
        return listSecondScientist;
    }

    final int x = 1;
    final int y = 4;

    public SecondScientist(ResourceFactory resourceFactory) {
    ...
    public void run() {
        synchronized (resourceFactory) {
            ...
            System.out.println("Количество запчастей на свалке c приходом 2-го : " + resourceFactory.getListDetail().size());
    ...
    public void collectionOfParts(List<SpareParts> list, int numberOfTaken) {
        System.out.println("SecondScientist.Забирает в ночь: " + numberOfTaken);
        if (list.size() == 0) {
            System.out.println("SecondScientist ушёл ни с чем. На свалке " + list.size() + " деталей");
        } else if (numberOfTaken <= list.size()) {
            for (int t = 0; t < numberOfTaken; t++) {
                int randomIndexDetail = (int) (Math.random() * ((list.size()-1) + 1));
                listSecondScientist.add(list.get(randomIndexDetail));
                list.remove(randomIndexDetail);
            }
            System.out.println("Thread 2: " + listSecondScientist.size());
        } else {
